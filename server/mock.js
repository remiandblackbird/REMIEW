/*
 * Fallback used automatically when ANTHROPIC_API_KEY is not set, so the app
 * still runs end-to-end without a key. See server/routes.js for the switch.
 */
...
async function reviewLabel({ fileName }) {
  await delay(600);
  return {
...
async function extractSpec({ fileName }) {
  await delay(500);
  const hit = SPEC_TEMPLATES.find((t) => t.match.test(fileName || ''));
  if (hit) {
...
async function generateDeclaration(names) {
  await delay(400);
...
async function checkClaim(claimText, nip) {
  await delay(500);
...
async function chatReply(history, moduleLabel) {
  await delay(400);
  const list = history || [];
  ...
  return `This server is running without an ANTHROPIC_API_KEY, so I can't give a real answer about "${moduleLabel}" topics yet. Add a key to your .env file to get real answers here.`;
}

module.exports = { reviewLabel, extractSpec, generateDeclaration, checkClaim, chatReply };
